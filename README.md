...
*   **关键教训**:
  ...
  2025.6.14
  多次重启预训练过程，第一次的失误是mtp模块的实现问题，第二次失误是仅使用了一个批次，导致模型不收敛，现在重启的还行
  后来反思才发现，模型在训练多个batchsize的数据时，才会对各类文本有一个通识，事实上batchsize在一定程度上决定了模型rollout出来的文本多样性

  最近也有阅读一些论文，不过现在感觉，rl难做，一定程度上是因为gpu的问题，还有就是算法优化，这也涉及到了infra工程，像什么模型并行，张量并行，专家并行等等，都需要优化

  事实上，如果我后面需要写一个有一些影响力的论文，那么我就需要对算法有些改进，最好是有独到之处的改进

  另一方面，我看到一篇论文是使用强化学习进行预训练的，当时我就问过ai，这样的预测方式其实是无偏估计，但是也存在很高的方差，以及训练效率的问题，其实也是
  当大规模拓展这一方案的话，说不定模型会
## 当前挑战与思考
