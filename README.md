...
- [项目进展日志](#项目进展日志)
  - [当前实验：融合 Gemma-3N 技术的混合架构探索 (2025-06-24)](#当前实验融合-gemma-3n-技术的混合架构探索-2025-06-24)
  - [历史日志](#历史日志)
- [当前挑战与思考](#当前挑战与思考)
- [未来计划](#未来计划)
- [如何贡献](#如何贡献)
...
### 当前实验：融合 Gemma-3N 技术的混合架构探索 (2025-06-24)

这是第四次重启预训练过程。本次实验不再是单纯复现，而是转向了对一个**混合架构**的探索，其灵感主要来源于对 [Gemma-3N 逆向工程](https://github.com/antimatter15/reverse-engineering-gemma-3n) 的研究。

*   **架构创新**:
    *   **核心架构**: 融合了类 Gemini 架构的设计理念。
    *   **关键技术**:
        1.  引入了 Gemma-3N 的 `per_layer_embed` 技术。
        2.  采用了增强的残差连接层 (`enhanced residual connections`)。
        3.  继续沿用 `MTP` (Multi-token Prediction) 技术以加速训练。
        4.  集成了专家负载均衡损失 (`expert load balancing loss`) 来优化 MoE 路由。
*   **初步观察**:
    *   **损失显著下降**: 在初步测试中，新架构的模型损失有明显下降。
    *   **核心问题**: 目前尚待探索——性能的提升究竟是源于**更优的架构设计**，还是仅仅因为**参数量扩大**带来的增益？

### 历史日志

**2025-06-14**
*   **核心事件**: 多次重启预训练，总结了前几次的失败教训。
*   **关键教训**:
    1.  **实现细节是魔鬼**: 第一次失败源于 `MTP` 模块的实现问题。
    2.  **Batch Size 的重要性**: 第二次失败是由于训练时 Batch Size 过小（可能为1），导致模型无法学习到数据的通用模式，不收敛。**足够大的 Batch Size 对于模型泛化能力和生成文本的多样性至关重要。**
*   **个人思考**:
    *   强化学习的瓶颈很大程度上是**算力 (GPU)** 和**工程优化 (Infra)** 问题，涉及模型并行、张量并行、专家并行等复杂技术。
    *   思考了通过强化学习进行预训练的论文，认识到这种方法虽然可能是无偏估计，但面临**高方差**和**训练效率低下**的挑战。

**2025-06-09**
*   **核心事件**: 尝试复现一个基于 DeepSeek-V3 架构的迷你模型，重点引入 `MTP` 和 `aux_loss`。
*   **训练方案**:
    *   **架构**: 自行实现了包含 `MTP` 和 `aux_loss` 的 DeepSeek-V3 兼容架构。
    *   **精度与优化**: 采用 **FlashAttention + bfloat16 混合精度**，在单张 `RTX 4060Ti` 上训练。
*   **日志分析 (截至 ~130k steps)**:
    *   `train/loss` 曲线健康下降，证明 `MTP` 和 `aux_loss` 对稳定训练有效。
    *   在约 60k 步时出现**梯度尖峰 (spike)**，虽然被优化器拉回，但暴露了训练稳定性的挑战。
...
- [ ] 完成基于 **Gemma-3N 混合架构** 的新一轮预训练实验。
- [ ] 进行消融实验，分析性能增益来源于**架构改进**还是**参数增加**。
- [ ] 整合并开源包含 `MTP`, `per_layer_embed` 等技术的训练脚本和配置文件。
- [ ] 发布预训练完成后的模型权重及详细的性能评测报告。
- [ ] 探索在当前模型基础上进行指令微调和人类偏好对齐（如 DPO）的有效方法。
- [ ] 撰写更详细的技术博客，分享混合架构、MTP、`aux_loss` 等模块的实现心得。
