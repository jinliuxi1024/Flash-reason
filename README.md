...
#### **2025-08-27: 上下文扩展与强化学习瓶颈分析**
*   **核心进展**:
    *   成功将模型的预训练上下文从前期检查点扩展到了 **4096**。
    *   在扩展上下文后的模型中发现了有趣的**泛化能力**。例如，模型在预训练中学习了“选择题+解释”的模式，现在有时能摆脱选择题的束缚，直接回答相关问题。
*   **强化学习实验与瓶颈**:
    *   尝试将 `gemma3-270M` 指令模型微调成推理模型，再通过强化学习进行微调，目标是裁剪掉不必要的探索性回答，使输出更精简。
    *   **遭遇显存瓶颈**：由于 Gemma3 的词表（约 256k）是 DeepSeek（约 128k）的两倍多，导致在 RL 阶段存储 `rollout` 数据时需要更多显存。
    *   **妥协与影响**: 不得不减少每次 `rollout` 的数量，这直接影响了强化学习的性能。该问题有待未来计算资源升级后解决。
*   **新的理论思考**:
    *   开始反思自回归模型推理能力提升的根本原因。一个核心问题是：**模型是否必须经过输出前预计算（指当前的推理->输出范式）才能获得强大的推理能力？**
    *   对 RL 效果的审视：如果强化学习真的普遍有效，其带来的性能提升应当远不止于将 `pass@1` 的成功率提升到 `pass@64` 的水平。这暗示了当前 RL 方法在根本上提升模型核心推理能力方面可能存在局限。

#### **2024-08-17: 预训练进展与混合推理架构探索**
*   **核心事件**:
    *   预训练取得显著进展，模型生成的文本质量得到大幅提升。
    *   深入研究 `gpt-oss` 模型实现，并对其中的 `attn-sink` 机制进行了学习。
    *   开始设计一种新的混合推理模型架构。
*   **关键洞察与新思路**:
    *   **Attn-Sink 机制**: `gpt-oss` 引入的 `attn-sink` 机制启发了我。该设计源于一篇论文的发现：当大语言模型对生成内容不确定时，softmax 机制会强制将注意力分数总和分配为 1，导致注意力被错误地集中在初始的、可能无意义的 token 上。`attn-sink` 就像一个注意力池，解决了这个问题，对提升生成质量至关重要。
    *   **混合推理模型构想**: 我正在构思一种将**扩散语言模型**与**自回归语言模型**相结合的混合生成架构。其核心思想是在进行扩散更新的同时实现自回归生成，期望通过这种方式加速推理速度。目前，这个模型的具体架构设计正在打磨中。
...
#### 对强化学习瓶颈的再思考：资源与理论
最近的实验（如使用 Gemma3 进行 RL）再次暴露了资源对强化学习的强约束。Gemma3 巨大的词表直接导致了 `rollout` 阶段的显存爆炸，迫使减少探索批次，从而损害了训练效果。这再次印证了“强化学习的瓶颈不仅在于算法本身，更深层次地受到 GPU 资源和系统工程...的制约”这一观点。

此外，这也引发了更深层次的理论思考：我们当前通过 SFT -> RL 提升推理能力的范式是否是唯一的路径？如果 RL 的作用主要是将模型已有的、通过 `pass@k` 才能体现的潜在能力“提纯”到 `pass@1`，那么它在多大程度上是在“教会”模型新的推理能力，又在多大程度上只是在做“表达优化”？对这个问题的探索，可能是未来提升模型智能水平的关键。
